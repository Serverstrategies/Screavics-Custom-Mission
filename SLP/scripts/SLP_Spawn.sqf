...
_task =  if (count _this >= 8) then {_this select 7} else {nil} ;
_string = if (count _this >= 9) then {_this select 8} else {false};
_debug = SLP_ARRAY select 0;

if (count _this < 7) exitwith {hintc format [ "Error spawnname %1: A SLP script call does not have enough elements\n\nit should have at least 7 elements \n\n0 =[""spawnname"",[Side,Faction],[spawn position],[units],[veh],[tank],[air]] spawn SLP_Spawn\n\nScript call aborted", _spawnname]};
...
if (_inf) then {
	...
	[_spawnname,_side,_delay,_position,_temparray,_infskill,_infgrp,_task,_string] spawn SLP_fnc_Infantry;
	} else {
		if (_debug==1) then {          
...
if (_veh) then {
    if ( count _vehicles >=1) then {
  	[_spawnname,_side,_delay,_position,_temparray,_infskill,_vehskill,_vehgrp,_task,_string,0] spawn SLP_fnc_vehicle;
	} else {
		if (_debug==1) then {
...
if (_arm) then {
    if ( count _armor >=1) then {
  	[_spawnname,_side,_delay,_position,_temparray,_infskill,_vehskill,_armgrp,_task,_string,1] spawn SLP_fnc_vehicle;
	} else {
		if (_debug==1) then {      
...
sleep 1;
 ...
 if (_air) then {
	...
	[_spawnname,_side,_delay,_position,_temparray,_airskill,_airgrp,_task,_string] spawn SLP_fnc_air;
	} else {
		if (_debug==1) then {
